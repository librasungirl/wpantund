...
void SpinelNCPInstance::handle_ncp_debug_stream(const uint8_t *data_ptr, int data_len)
{
    ...
    while (data_len--)
    {
        ...
        if ((linepos != 0) && ((nextchar == '\n') || (nextchar == '\r') || (linepos >= (sizeof(linebuffer) - 1))))
        {
            // flush.
...
static const char *ot_log_level_to_string(uint8_t log_level)
{
    const char *retval = "----";
...
static const char *ot_log_region_to_string(unsigned int log_region)
{
    const char *retval = "---------";
...
void SpinelNCPInstance::handle_ncp_log_stream(const uint8_t *data_in, int data_len)
{
    spinel_ssize_t len;
    char           prefix_string[NCP_DEBUG_LINE_LENGTH_MAX + 1];
    const char *   log_string;

    len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_UTF8_S, &log_string);
    require(len >= 0, bail);
    ...
    if ((data_len > 0) && mCapabilities.count(SPINEL_CAP_OPENTHREAD_LOG_METADATA))
    {
        ...
        len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_UINT8_S SPINEL_DATATYPE_UINT_PACKED_S,
                                     &log_level, &log_region);

        require(len >= 0, bail);

        snprintf(prefix_string, sizeof(prefix_string), "[%s]%s: ", ot_log_level_to_string(log_level),
...
void SpinelNCPInstance::start_new_task(const boost::shared_ptr<SpinelNCPTask> &task)
{
    ...
    {
        if (ncp_state_is_sleeping(get_ncp_state()) && (dynamic_cast<const SpinelNCPTaskWake *>(task.get()) == NULL))
        {
            if (can_set_ncp_power() || !mCapabilities.count(SPINEL_CAP_MCU_POWER_STATE))
            {
...
int nl::wpantund::spinel_status_to_wpantund_status(int spinel_status)
{
...
int nl::wpantund::peek_ncp_callback_status(int event, va_list args)
{
    int ret = 0;
...
SpinelNCPInstance::SpinelNCPInstance(const Settings &settings)
    : NCPInstanceBase(settings)
    , mControlInterface(this)
    , mVendorCustom(this)
{
    ...
    if (!settings.empty())
    {
        ...
        for (iter = settings.begin(); iter != settings.end(); iter++)
        {
            if (!NCPInstanceBase::setup_property_supported_by_class(iter->first))
            {
                ...
                if (status != 0)
                {
                    syslog(LOG_WARNING, "Attempt to set property \"%s\" failed with err %d", iter->first.c_str(),
                           status);
                }
            }
        }
...
std::string SpinelNCPInstance::thread_mode_to_string(uint8_t mode)
{
    char c_string[400];

    snprintf(c_string, sizeof(c_string), "RxOnWhenIdle:%s FTD:%s FullNetData:%s SecDataReq:%s",
...
uint8_t SpinelNCPInstance::get_thread_mode(void)
{
    return mThreadMode;
}

bool SpinelNCPInstance::setup_property_supported_by_class(const std::string &prop_name)
{
    return NCPInstanceBase::setup_property_supported_by_class(prop_name);
}

SpinelNCPControlInterface &SpinelNCPInstance::get_control_interface()
{
    return mControlInterface;
}

std::set<std::string> SpinelNCPInstance::get_supported_property_keys() const
{
...
cms_t SpinelNCPInstance::get_ms_to_next_event(void)
{
...
static int unpack_commissioner_state(const uint8_t *data_in, spinel_size_t data_len, boost::any &value)
{
    spinel_ssize_t len;
    uint8_t        state;
    int            ret = kWPANTUNDStatus_Ok;

    len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_UINT8_S, &state);
...
static int unpack_channel_mask(const uint8_t *data_in, spinel_size_t data_len, boost::any &value)
{
    spinel_ssize_t len;
    ...
    while (data_len > 0)
    {
        len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_UINT8_S, &channel);
...
static int unpack_mac_whitelist_entries(const uint8_t *data_in,
                                        ...
                                        bool           as_val_map)
{
    ...
    while (data_len > 0)
    {
        len = spinel_datatype_unpack(data_in, data_len,
                                     SPINEL_DATATYPE_STRUCT_S(SPINEL_DATATYPE_EUI64_S    // Extended address
        ...
        else
        {
            ...
            index = snprintf(c_string, sizeof(c_string), "%02X%02X%02X%02X%02X%02X%02X%02X", eui64->bytes[0],
                             eui64->bytes[1], eui64->bytes[2], eui64->bytes[3], eui64->bytes[4], eui64->bytes[5],
                             eui64->bytes[6], eui64->bytes[7]);
...
static int unpack_mac_blacklist_entries(const uint8_t *data_in,
                                        spinel_size_t  data_len,
                                        boost::any &   value,
                                        bool           as_val_map)
{
...
static int unpack_channel_occupancy(const uint8_t *data_in, spinel_size_t data_len, boost::any &value, bool as_val_map)
{
    std::list<std::string> result_as_string;
    std::list<ValueMap>    result_as_val_map;
    int                    ret = kWPANTUNDStatus_Ok;
    ...
    while (data_len > 0)
    {
        ...
        len = spinel_datatype_unpack(data_in, data_len,
                                     SPINEL_DATATYPE_STRUCT_S(SPINEL_DATATYPE_UINT8_S      // Channel
...
static int unpack_ncp_counters_all_mac(const uint8_t *data_in,
                                       spinel_size_t  data_len,
                                       boost::any &   value,
                                       bool           as_val_map)
{
    ...
    const char *tx_counter_names[] = {
        kWPANTUNDValueMapKey_Counter_TxTotal,          kWPANTUNDValueMapKey_Counter_TxUnicast,
        kWPANTUNDValueMapKey_Counter_TxBroadcast,      kWPANTUNDValueMapKey_Counter_TxAckRequested,
        kWPANTUNDValueMapKey_Counter_TxAcked,          kWPANTUNDValueMapKey_Counter_TxNoAckRequested,
        kWPANTUNDValueMapKey_Counter_TxData,           kWPANTUNDValueMapKey_Counter_TxDataPoll,
        kWPANTUNDValueMapKey_Counter_TxBeacon,         kWPANTUNDValueMapKey_Counter_TxBeaconRequest,
        kWPANTUNDValueMapKey_Counter_TxOther,          kWPANTUNDValueMapKey_Counter_TxRetry,
        kWPANTUNDValueMapKey_Counter_TxErrCca,         kWPANTUNDValueMapKey_Counter_TxErrAbort,
        kWPANTUNDValueMapKey_Counter_TxErrBusyChannel, NULL};

    const char *rx_counter_names[] = {kWPANTUNDValueMapKey_Counter_RxTotal,
    ...
    for (int struct_index = 0; struct_index < 2; struct_index++)
    {
        ...
        len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_DATA_WLEN_S, &struct_in, &struct_len);
        ...
        while (*counter_names != NULL)
        {
            ...
            len = spinel_datatype_unpack(struct_in, struct_len, SPINEL_DATATYPE_UINT32_S, &counter_value);
...
static int unpack_ncp_counters_mle(const uint8_t *data_in, spinel_size_t data_len, boost::any &value, bool as_val_map)
{
    std::list<std::string> result_as_string;
    ValueMap               result_as_val_map;
    int                    ret = kWPANTUNDStatus_Ok;
    ...
    const char *mle_counter_names[] = {
        kWPANTUNDValueMapKey_MleCounter_DisabledRole,       kWPANTUNDValueMapKey_MleCounter_DetachedRole,
        kWPANTUNDValueMapKey_MleCounter_ChildRole,          kWPANTUNDValueMapKey_MleCounter_RouterRole,
        kWPANTUNDValueMapKey_MleCounter_LeaderRole,         kWPANTUNDValueMapKey_MleCounter_AttachAttempts,
        kWPANTUNDValueMapKey_MleCounter_PartitionIdChanges, kWPANTUNDValueMapKey_MleCounter_BetterPartitionAttaches,
        kWPANTUNDValueMapKey_MleCounter_ParentChanges,      NULL};
    ...
    while (*counter_names != NULL)
    {
        ...
        len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_UINT16_S, &counter_value);
...
static int unpack_mcu_power_state(const uint8_t *data_in, spinel_size_t data_len, boost::any &value)
{
    spinel_ssize_t len;
    uint8_t        power_state;
    int            ret = kWPANTUNDStatus_Ok;

    len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_UINT8_S, &power_state);
...
static int unpack_parent_info(const uint8_t *data_in, spinel_size_t data_len, boost::any &value, bool as_val_map)
{
    ...
    len = spinel_datatype_unpack(data_in, data_len,
                                 ...
                                 &eui64, &rloc16, &age, &average_rssi, &last_rssi, &lqin, &lqout);
    ...
    if (!as_val_map)
    {
        ...
        snprintf(c_string, sizeof(c_string),
                 ...
                 "LQIn:%d, "
                 "LQOut:%d",
                 eui64->bytes[0], eui64->bytes[1], eui64->bytes[2], eui64->bytes[3], eui64->bytes[4], eui64->bytes[5],
                 eui64->bytes[6], eui64->bytes[7], rloc16, age, average_rssi, last_rssi, lqin, lqout);
...
static int unpack_address_cache_table(const uint8_t *data_in,
                                      spinel_size_t  data_len,
                                      boost::any &   value,
                                      bool           as_val_map)
{
    ...
    while (data_len > 0)
    {
        ...
        len = spinel_datatype_unpack(
            data_in, data_len,
            SPINEL_DATATYPE_STRUCT_S(SPINEL_DATATYPE_IPv6ADDR_S SPINEL_DATATYPE_UINT16_S SPINEL_DATATYPE_UINT8_S),
            &target_address, &target_rloc16, &age);
        ...
        if (!as_val_map)
        {
            ...
            snprintf(c_string, sizeof(c_string), "%s -> 0x%04x, age:%d", in6_addr_to_string(*target_address).c_str(),
                     target_rloc16, age);
...
static int unpack_mesh_local_prefix(const uint8_t *data_in, spinel_size_t data_len, boost::any &value)
{
    spinel_ssize_t   len;
    ...
    len = spinel_datatype_unpack(data_in, data_len, (SPINEL_DATATYPE_IPv6ADDR_S SPINEL_DATATYPE_UINT8_S), &addr,
                                 &prefix_len);
...
static int unpack_dataset(const uint8_t *data_in, spinel_size_t data_len, boost::any &value, bool as_val_map)
{
    int                    ret = kWPANTUNDStatus_Ok;
...
static int unpack_server_leader_services_as_any(const uint8_t *data_in,
                                                spinel_size_t  data_len,
                                                boost::any &   value,
                                                bool           as_val_map)
{
    ...
    {
        len = spinel_datatype_unpack(data_in, data_len,
                                     SPINEL_DATATYPE_STRUCT_S(SPINEL_DATATYPE_UINT8_S                      // Service ID
                                     ...
                                     &service_id, &enterprise_number, &service_data, &service_data_len, &stable,
                                     &server_data, &server_data_len, &rloc16);
        ...
        {
            snprintf(c_string, sizeof(c_string), "ServiceId:%01x, EnterpriseNumber:%u, Stable:%d, RLOC16:%04x",
                     service_id, enterprise_number, stable, rloc16);
...
static int unpack_meshcop_joiner_state(const uint8_t *data_in, spinel_size_t data_len, boost::any &value)
{
    spinel_ssize_t len;
    uint8_t        joiner_state;
    int            ret = kWPANTUNDStatus_Ok;

    len = spinel_datatype_unpack(data_in, data_len, SPINEL_DATATYPE_UINT8_S, &joiner_state);
...
static int unpack_domain_prefix(const uint8_t *data_in, spinel_size_t data_len, boost::any &value)
{
    spinel_ssize_t   len;
    struct in6_addr *addr;
    uint8_t          prefix_len;
...
static int unpack_thread_network_time_spinel(const uint8_t *data_in,
                                             spinel_size_t  data_len,
                                             uint64_t &     time,
                                             int8_t &       time_sync_status)
{
    return spinel_datatype_unpack(data_in, data_len,
                                  (SPINEL_DATATYPE_UINT64_S   // time
                                       SPINEL_DATATYPE_INT8_S // time sync status
                                   ),
                                  &time, &time_sync_status);
}

static int unpack_thread_network_time_as_string(const uint8_t *data_in, spinel_size_t data_len, std::string &result)
{
    spinel_ssize_t len;
    uint64_t       time;
    int8_t         time_sync_status;
    ...
    {
        ret = kWPANTUNDStatus_Ok;
        snprintf(c_string, sizeof(c_string), "ThreadNetworkTime: %" PRIu64 ", TimeSyncStatus:%d", time,
                 time_sync_status);
...
static int unpack_thread_network_time_as_valmap(const uint8_t *data_in, spinel_size_t data_len, ValueMap &result)
{
    spinel_ssize_t len;
    ValueMap       entry;
    uint64_t       time;
...
static int unpack_thread_network_time_as_any(const uint8_t *data_in,
                                             spinel_size_t  data_len,
                                             boost::any &   value,
                                             bool           as_val_map)
{
...
void SpinelNCPInstance::get_dataset_command_help(std::list<std::string> &list)
{
    list.clear();
    ...
    list.push_back("   - `" kWPANTUNDDatasetCommand_GetActive
                   "`: Get the NCP's Active Operational Dataset and populate the local Dataset from it");
    list.push_back("   - `" kWPANTUNDDatasetCommand_SetActive
                   "`: Set the NCP's Active Operational Dataset from the current local Dataset");
    list.push_back("   - `" kWPANTUNDDatasetCommand_SendMgmtGetActive
                   "`: Send MGMT_GET_ACTIVE meshcop command requesting TLVs in current local Dataset");
    list.push_back("   - `" kWPANTUNDDatasetCommand_SendMgmtSetActive
                   "`: Send MGMT_SET_ACTIVE meshcop command along with the current local Dataset");
    list.push_back("   - `" kWPANTUNDDatasetCommand_GetPending
                   "`: Get the NCP's Pending Operational Dataset and populate the local DataSet from it");
    list.push_back("   - `" kWPANTUNDDatasetCommand_SetPending
                   "`: Set the NCP's Pending Operational Dataset from the current local Dataset");
    list.push_back("   - `" kWPANTUNDDatasetCommand_SendMgmtGetPending
                   "`: Send MGMT_GET_PENDING meshcop command requesting TLVs in the current local Dataset");
    list.push_back("   - `" kWPANTUNDDatasetCommand_SendMgmtSetPending
                   "`: Send MGMT_SET_PENDING meshcop command along with the current local Dataset");
}

int SpinelNCPInstance::unpack_and_set_local_dataset(const uint8_t *data_in, spinel_size_t data_len)
{
    return mLocalDataset.set_from_spinel_frame(data_in, data_len);
}

void SpinelNCPInstance::perform_dataset_command(const std::string &command, CallbackWithStatus cb)
{
    ...
    {
        get_spinel_prop_with_unpacker(boost::bind(cb, _1), SPINEL_PROP_THREAD_ACTIVE_DATASET,
    ...
    {
        get_spinel_prop_with_unpacker(boost::bind(cb, _1), SPINEL_PROP_THREAD_PENDING_DATASET,
                                      boost::bind(&SpinelNCPInstance::unpack_and_set_local_dataset, this, _1, _2));
    }
...
void SpinelNCPInstance::update_node_type(NodeType new_node_type)
{
    if (mNodeType != new_node_type)
    {
        syslog(LOG_NOTICE, "Node type change: \"%s\" -> \"%s\"", node_type_to_string(mNodeType).c_str(),
...
void SpinelNCPInstance::update_link_local_address(struct in6_addr *addr)
{
    if (NULL != addr && (0 != memcmp(mNCPLinkLocalAddress.s6_addr, addr->s6_addr, sizeof(mNCPLinkLocalAddress))))
    {
...
void SpinelNCPInstance::update_mesh_local_address(struct in6_addr *addr)
{
    if (addr && buffer_is_nonzero(addr->s6_addr, 8) &&
...
void SpinelNCPInstance::update_mesh_local_prefix(struct in6_addr *addr)
{
    if (addr && buffer_is_nonzero(addr->s6_addr, 8) && (0 != memcmp(mNCPV6Prefix, addr, sizeof(mNCPV6Prefix))))
    {
...
void SpinelNCPInstance::get_spinel_prop(CallbackWithStatusArg1 cb,
                                        spinel_prop_key_t      prop_key,
...
void SpinelNCPInstance::get_spinel_prop_with_unpacker(CallbackWithStatusArg1 cb,
                                                      spinel_prop_key_t      prop_key,
...
void SpinelNCPInstance::check_capability_prop_get(CallbackWithStatusArg1 cb,
                                                  ...
                                                  PropGetHandler         handler)
{
    ...
    {
        char error_msg[200];
        snprintf(error_msg, sizeof(error_msg), "Capability %s (required for \"%s\") is not supported by NCP",
                 spinel_capability_to_cstr(capability), prop_name.c_str());
        cb(kWPANTUNDStatus_FeatureNotSupported, std::string(error_msg));
    }
}

void SpinelNCPInstance::register_get_handler(const char *prop_name, PropGetHandler handler)
{
    NCPInstanceBase::register_prop_get_handler(prop_name, handler);
}

void SpinelNCPInstance::register_get_handler_capability(const char *   prop_name,
                                                        unsigned int   capability,
                                                        PropGetHandler handler)
{
...
void SpinelNCPInstance::register_get_handler_spinel_simple(const char *      prop_name,
                                                           spinel_prop_key_t prop_key,
                                                           const char *      reply_format)
{
    register_get_handler(
        prop_name, boost::bind(&SpinelNCPInstance::get_spinel_prop, this, _1, prop_key, std::string(reply_format)));
}

void SpinelNCPInstance::register_get_handler_spinel_unpacker(const char *      prop_name,
                                                             spinel_prop_key_t prop_key,
                                                             ReplyUnpacker     unpacker)
{
    register_get_handler(prop_name,
...
void SpinelNCPInstance::register_get_handler_capability_spinel_simple(const char *      prop_name,
                                                                      unsigned int      capability,
                                                                      spinel_prop_key_t prop_key,
                                                                      const char *      reply_format)
{
    register_get_handler_capability(
        prop_name, capability,
        boost::bind(&SpinelNCPInstance::get_spinel_prop, this, _1, prop_key, std::string(reply_format)));
}

void SpinelNCPInstance::register_get_handler_capability_spinel_unpacker(const char *      prop_name,
                                                                        unsigned int      capability,
                                                                        spinel_prop_key_t prop_key,
                                                                        ReplyUnpacker     unpacker)
{
...
void SpinelNCPInstance::regsiter_all_get_handlers(void)
{
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // Properties associated with a spinel property with simple packing format

    register_get_handler_spinel_simple(kWPANTUNDProperty_NCPCCAThreshold, SPINEL_PROP_PHY_CCA_THRESHOLD,
                                       SPINEL_DATATYPE_INT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NCPTXPower, SPINEL_PROP_PHY_TX_POWER, SPINEL_DATATYPE_INT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NCPFrequency, SPINEL_PROP_PHY_FREQ, SPINEL_DATATYPE_INT32_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NetworkKey, SPINEL_PROP_NET_MASTER_KEY,
                                       SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NetworkPSKc, SPINEL_PROP_NET_PSKC, SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NCPExtendedAddress, SPINEL_PROP_MAC_EXTENDED_ADDR,
                                       SPINEL_DATATYPE_EUI64_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NetworkKeyIndex, SPINEL_PROP_NET_KEY_SEQUENCE_COUNTER,
                                       SPINEL_DATATYPE_UINT32_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NetworkKeySwitchGuardTime,
                                       SPINEL_PROP_NET_KEY_SWITCH_GUARDTIME, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NetworkRole, SPINEL_PROP_NET_ROLE, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NetworkPartitionId, SPINEL_PROP_NET_PARTITION_ID,
                                       SPINEL_DATATYPE_UINT32_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadRouterUpgradeThreshold,
                                       SPINEL_PROP_THREAD_ROUTER_UPGRADE_THRESHOLD, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadRouterDowngradeThreshold,
                                       SPINEL_PROP_THREAD_ROUTER_DOWNGRADE_THRESHOLD, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NCPRSSI, SPINEL_PROP_PHY_RSSI, SPINEL_DATATYPE_INT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadRLOC16, SPINEL_PROP_THREAD_RLOC16,
                                       SPINEL_DATATYPE_UINT16_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadRouterSelectionJitter,
                                       SPINEL_PROP_THREAD_ROUTER_SELECTION_JITTER, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadLeaderAddress, SPINEL_PROP_THREAD_LEADER_ADDR,
                                       SPINEL_DATATYPE_IPv6ADDR_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadLeaderRouterID, SPINEL_PROP_THREAD_LEADER_RID,
                                       SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadLeaderWeight, SPINEL_PROP_THREAD_LEADER_WEIGHT,
                                       SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadLeaderLocalWeight,
                                       SPINEL_PROP_THREAD_LOCAL_LEADER_WEIGHT, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadNetworkData, SPINEL_PROP_THREAD_NETWORK_DATA,
                                       SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadNetworkDataVersion,
                                       SPINEL_PROP_THREAD_NETWORK_DATA_VERSION, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadStableNetworkData,
                                       SPINEL_PROP_THREAD_STABLE_NETWORK_DATA, SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadLeaderNetworkData,
                                       SPINEL_PROP_THREAD_LEADER_NETWORK_DATA, SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadStableLeaderNetworkData,
                                       SPINEL_PROP_THREAD_STABLE_LEADER_NETWORK_DATA, SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadStableNetworkDataVersion,
                                       SPINEL_PROP_THREAD_STABLE_NETWORK_DATA_VERSION, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadRouterRoleEnabled,
                                       SPINEL_PROP_THREAD_ROUTER_ROLE_ENABLED, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadDeviceMode, SPINEL_PROP_THREAD_MODE,
                                       SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_OpenThreadDebugTestAssert, SPINEL_PROP_DEBUG_TEST_ASSERT,
                                       SPINEL_DATATYPE_BOOL_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_OpenThreadDebugTestWatchdog, SPINEL_PROP_DEBUG_TEST_WATCHDOG,
                                       SPINEL_DATATYPE_BOOL_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_TmfProxyEnabled, SPINEL_PROP_THREAD_TMF_PROXY_ENABLED,
                                       SPINEL_DATATYPE_BOOL_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_NCPCCAFailureRate, SPINEL_PROP_MAC_CCA_FAILURE_RATE,
                                       SPINEL_DATATYPE_UINT16_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadChildTimeout, SPINEL_PROP_THREAD_CHILD_TIMEOUT,
                                       SPINEL_DATATYPE_UINT32_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_OpenThreadLogLevel, SPINEL_PROP_DEBUG_NCP_LOG_LEVEL,
                                       SPINEL_DATATYPE_UINT8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadDomainName, SPINEL_PROP_THREAD_DOMAIN_NAME,
                                       SPINEL_DATATYPE_UTF8_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadBackboneInterface, SPINEL_PROP_THREAD_BACKBONE_INTERFACE,
                                       SPINEL_DATATYPE_BOOL_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadBackboneCoapPort, SPINEL_PROP_THREAD_BACKBONE_COAP_PORT,
                                       SPINEL_DATATYPE_UINT16_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadPrimaryState,
                                       SPINEL_PROP_THREAD_BACKBONE_ROUTER_PRIMARY_STATE, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadBbrGroupTable, SPINEL_PROP_THREAD_GROUP_TABLE,
                                       SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadBbrNdProxyTable, SPINEL_PROP_THREAD_NDPROXY_TABLE,
                                       SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadLocalBbr, SPINEL_PROP_THREAD_BACKBONE_ROUTER_LOCAL,
                                       SPINEL_DATATYPE_DATA_S);
    register_get_handler_spinel_simple(kWPANTUNDProperty_ThreadPrimaryBbr, SPINEL_PROP_THREAD_BACKBONE_ROUTER_PRIMARY,
                                       SPINEL_DATATYPE_DATA_S);
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // Properties requiring capability check and associated with a spinel property
    // with simple packing format

    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPSleepyPollInterval, SPINEL_CAP_ROLE_SLEEPY,
                                                  SPINEL_PROP_MAC_DATA_POLL_PERIOD, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(
        kWPANTUNDProperty_CommissionerProvisioningUrl, SPINEL_CAP_THREAD_COMMISSIONER,
        SPINEL_PROP_MESHCOP_COMMISSIONER_PROVISIONING_URL, SPINEL_DATATYPE_UTF8_S);
    register_get_handler_capability_spinel_simple(
        kWPANTUNDProperty_CommissionerSessionId, SPINEL_CAP_THREAD_COMMISSIONER,
        SPINEL_PROP_MESHCOP_COMMISSIONER_SESSION_ID, SPINEL_DATATYPE_UINT16_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_MACWhitelistEnabled, SPINEL_CAP_MAC_WHITELIST,
                                                  SPINEL_PROP_MAC_WHITELIST_ENABLED, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_MACBlacklistEnabled, SPINEL_CAP_MAC_WHITELIST,
                                                  SPINEL_PROP_MAC_BLACKLIST_ENABLED, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_JamDetectionStatus, SPINEL_CAP_JAM_DETECT,
                                                  SPINEL_PROP_JAM_DETECTED, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_JamDetectionEnable, SPINEL_CAP_JAM_DETECT,
                                                  SPINEL_PROP_JAM_DETECT_ENABLE, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_JamDetectionRssiThreshold, SPINEL_CAP_JAM_DETECT,
                                                  SPINEL_PROP_JAM_DETECT_RSSI_THRESHOLD, SPINEL_DATATYPE_INT8_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_JamDetectionWindow, SPINEL_CAP_JAM_DETECT,
                                                  SPINEL_PROP_JAM_DETECT_WINDOW, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_JamDetectionBusyPeriod, SPINEL_CAP_JAM_DETECT,
                                                  SPINEL_PROP_JAM_DETECT_BUSY, SPINEL_DATATYPE_UINT8_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_JamDetectionDebugHistoryBitmap,
                                                  SPINEL_CAP_JAM_DETECT, SPINEL_PROP_JAM_DETECT_HISTORY_BITMAP,
                                                  SPINEL_DATATYPE_UINT64_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_ChildSupervisionInterval,
                                                  SPINEL_CAP_CHILD_SUPERVISION, SPINEL_PROP_CHILD_SUPERVISION_INTERVAL,
                                                  SPINEL_DATATYPE_UINT16_S);
    register_get_handler_capability_spinel_simple(
        kWPANTUNDProperty_ChildSupervisionCheckTimeout, SPINEL_CAP_CHILD_SUPERVISION,
        SPINEL_PROP_CHILD_SUPERVISION_CHECK_TIMEOUT, SPINEL_DATATYPE_UINT16_S);
    register_get_handler_capability_spinel_simple(
        kWPANTUNDProperty_ChannelMonitorSampleInterval, SPINEL_CAP_CHANNEL_MONITOR,
        SPINEL_PROP_CHANNEL_MONITOR_SAMPLE_INTERVAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_ChannelMonitorRssiThreshold,
                                                  SPINEL_CAP_CHANNEL_MONITOR,
                                                  SPINEL_PROP_CHANNEL_MONITOR_RSSI_THRESHOLD, SPINEL_DATATYPE_INT8_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_ChannelMonitorSampleWindow,
                                                  SPINEL_CAP_CHANNEL_MONITOR, SPINEL_PROP_CHANNEL_MONITOR_SAMPLE_WINDOW,
                                                  SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_ChannelMonitorSampleCount,
                                                  SPINEL_CAP_CHANNEL_MONITOR, SPINEL_PROP_CHANNEL_MONITOR_SAMPLE_COUNT,
                                                  SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_ChannelManagerNewChannel,
                                                  SPINEL_CAP_CHANNEL_MANAGER, SPINEL_PROP_CHANNEL_MANAGER_NEW_CHANNEL,
                                                  SPINEL_DATATYPE_UINT8_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_ChannelManagerDelay, SPINEL_CAP_CHANNEL_MANAGER,
                                                  SPINEL_PROP_CHANNEL_MANAGER_DELAY, SPINEL_DATATYPE_UINT16_S);
    register_get_handler_capability_spinel_simple(
        kWPANTUNDProperty_ChannelManagerAutoSelectEnabled, SPINEL_CAP_CHANNEL_MANAGER,
        SPINEL_PROP_CHANNEL_MANAGER_AUTO_SELECT_ENABLED, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_capability_spinel_simple(
        kWPANTUNDProperty_ChannelManagerAutoSelectInterval, SPINEL_CAP_CHANNEL_MANAGER,
        SPINEL_PROP_CHANNEL_MANAGER_AUTO_SELECT_INTERVAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_ChannelManagerChannelSelect,
                                                  SPINEL_CAP_CHANNEL_MANAGER,
                                                  SPINEL_PROP_CHANNEL_MANAGER_CHANNEL_SELECT, SPINEL_DATATYPE_BOOL_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NestLabs_LegacyMeshLocalPrefix,
                                                  SPINEL_CAP_NEST_LEGACY_INTERFACE, SPINEL_PROP_NEST_LEGACY_ULA_PREFIX,
                                                  SPINEL_DATATYPE_DATA_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_TimeSync_Period, SPINEL_CAP_TIME_SYNC,
                                                  SPINEL_PROP_TIME_SYNC_PERIOD, SPINEL_DATATYPE_UINT16_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_TimeSync_XtalThreshold, SPINEL_CAP_TIME_SYNC,
                                                  SPINEL_PROP_TIME_SYNC_XTAL_THRESHOLD, SPINEL_DATATYPE_UINT16_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_UNICAST, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_UNICAST, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_BROADCAST, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_BROADCAST, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_ACK_REQ, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_ACK_REQ, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_ACKED, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_ACKED, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_NO_ACK_REQ, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_NO_ACK_REQ, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_DATA, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_DATA, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_DATA_POLL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_DATA_POLL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_BEACON, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_BEACON, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_BEACON_REQ, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_BEACON_REQ, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_OTHER, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_OTHER, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_PKT_RETRY, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_PKT_RETRY, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_ERR_CCA, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_ERR_CCA, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_ERR_ABORT, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_ERR_ABORT, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_UNICAST, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_UNICAST, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_BROADCAST, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_BROADCAST, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_DATA, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_DATA, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_DATA_POLL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_DATA_POLL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_BEACON, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_BEACON, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_BEACON_REQ, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_BEACON_REQ, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_OTHER, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_OTHER, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_FILT_WL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_FILT_WL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_PKT_FILT_DA, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_PKT_FILT_DA, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_ERR_EMPTY, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_ERR_EMPTY, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_ERR_UKWN_NBR, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_ERR_UKWN_NBR, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_ERR_NVLD_SADDR, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_ERR_NVLD_SADDR, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_ERR_SECURITY, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_ERR_SECURITY, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_ERR_BAD_FCS, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_ERR_BAD_FCS, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_ERR_OTHER, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_ERR_OTHER, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_IP_SEC_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_IP_SEC_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_IP_INSEC_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_IP_INSEC_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_IP_DROPPED, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_IP_DROPPED, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_IP_SEC_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_IP_SEC_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_IP_INSEC_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_IP_INSEC_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_IP_DROPPED, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_IP_DROPPED, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_TX_SPINEL_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_TX_SPINEL_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_SPINEL_TOTAL, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_SPINEL_TOTAL, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_RX_SPINEL_ERR, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_RX_SPINEL_ERR, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_IP_TX_SUCCESS, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_IP_TX_SUCCESS, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_IP_RX_SUCCESS, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_IP_RX_SUCCESS, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_IP_TX_FAILURE, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_IP_TX_FAILURE, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_NCPCounter_IP_RX_FAILURE, SPINEL_CAP_COUNTERS,
                                                  SPINEL_PROP_CNTR_IP_RX_FAILURE, SPINEL_DATATYPE_UINT32_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_POSIXAppRCPVersion, SPINEL_CAP_POSIX_APP,
                                                  SPINEL_PROP_RCP_VERSION, SPINEL_DATATYPE_UTF8_S);
    register_get_handler_capability_spinel_simple(kWPANTUNDProperty_OpenThreadSLAACEnabled, SPINEL_CAP_SLAAC,
                                                  SPINEL_PROP_SLAAC_ENABLED, SPINEL_DATATYPE_BOOL_S);

    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // Properties associated with a spinel property using an unpacker

    register_get_handler_spinel_unpacker(kWPANTUNDProperty_NCPChannelMask, SPINEL_PROP_PHY_CHAN_SUPPORTED,
                                         unpack_channel_mask);
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_NCPPreferredChannelMask, SPINEL_PROP_PHY_CHAN_PREFERRED,
                                         unpack_channel_mask);
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadDomainPrefix, SPINEL_PROP_THREAD_DOMAIN_PREFIX,
                                         unpack_domain_prefix);
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadActiveDataset, SPINEL_PROP_THREAD_ACTIVE_DATASET,
                                         boost::bind(unpack_dataset, _1, _2, _3, /* as_val_map */ false));
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadActiveDatasetAsValMap,
                                         SPINEL_PROP_THREAD_ACTIVE_DATASET,
                                         boost::bind(unpack_dataset, _1, _2, _3, /* as_val_map */ true));
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadPendingDataset, SPINEL_PROP_THREAD_PENDING_DATASET,
                                         boost::bind(unpack_dataset, _1, _2, _3, /* as_val_map */ false));
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadPendingDatasetAsValMap,
                                         SPINEL_PROP_THREAD_PENDING_DATASET,
                                         boost::bind(unpack_dataset, _1, _2, _3, /* as_val_map */ true));
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadParent, SPINEL_PROP_THREAD_PARENT,
                                         boost::bind(unpack_parent_info, _1, _2, _3, /* as_val_map */ false));
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadParentAsValMap, SPINEL_PROP_THREAD_PARENT,
                                         boost::bind(unpack_parent_info, _1, _2, _3, /* as_val_map */ true));
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadAddressCacheTable,
                                         SPINEL_PROP_THREAD_ADDRESS_CACHE_TABLE,
                                         boost::bind(unpack_address_cache_table, _1, _2, _3, /* as_val_map */ false));
    register_get_handler_spinel_unpacker(kWPANTUNDProperty_ThreadAddressCacheTableAsValMap,
                                         SPINEL_PROP_THREAD_ADDRESS_CACHE_TABLE,
    ...
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_NCPMCUPowerState, SPINEL_CAP_MCU_POWER_STATE,
                                                    SPINEL_PROP_MCU_POWER_STATE, unpack_mcu_power_state);
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_ThreadJoinerState, SPINEL_CAP_THREAD_JOINER,
                                                    SPINEL_PROP_MESHCOP_JOINER_STATE, unpack_meshcop_joiner_state);
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_CommissionerState, SPINEL_CAP_THREAD_COMMISSIONER,
                                                    SPINEL_PROP_MESHCOP_COMMISSIONER_STATE, unpack_commissioner_state);
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_MACWhitelistEntries, SPINEL_CAP_MAC_WHITELIST,
                                                    SPINEL_PROP_MAC_WHITELIST,
                                                    boost::bind(unpack_mac_whitelist_entries, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_MACWhitelistEntriesAsValMap,
                                                    SPINEL_CAP_MAC_WHITELIST, SPINEL_PROP_MAC_WHITELIST,
                                                    boost::bind(unpack_mac_whitelist_entries, _1, _2, _3, true));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_MACBlacklistEntries, SPINEL_CAP_MAC_WHITELIST,
                                                    SPINEL_PROP_MAC_BLACKLIST,
                                                    boost::bind(unpack_mac_blacklist_entries, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_MACBlacklistEntriesAsValMap,
                                                    SPINEL_CAP_MAC_WHITELIST, SPINEL_PROP_MAC_BLACKLIST,
                                                    boost::bind(unpack_mac_blacklist_entries, _1, _2, _3, true));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_MACFilterEntries, SPINEL_CAP_MAC_WHITELIST,
                                                    SPINEL_PROP_MAC_FIXED_RSS,
                                                    boost::bind(unpack_mac_whitelist_entries, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_MACFilterEntriesAsValMap,
                                                    SPINEL_CAP_MAC_WHITELIST, SPINEL_PROP_MAC_FIXED_RSS,
                                                    boost::bind(unpack_mac_whitelist_entries, _1, _2, _3, true));
    register_get_handler_capability_spinel_unpacker(
        kWPANTUNDProperty_ChannelMonitorChannelQuality, SPINEL_CAP_CHANNEL_MONITOR,
        SPINEL_PROP_CHANNEL_MONITOR_CHANNEL_OCCUPANCY, boost::bind(unpack_channel_occupancy, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(
        kWPANTUNDProperty_ChannelMonitorChannelQualityAsValMap, SPINEL_CAP_CHANNEL_MONITOR,
        SPINEL_PROP_CHANNEL_MONITOR_CHANNEL_OCCUPANCY, boost::bind(unpack_channel_occupancy, _1, _2, _3, true));
    register_get_handler_capability_spinel_unpacker(
        kWPANTUNDProperty_ChannelManagerSupportedChannelMask, SPINEL_CAP_CHANNEL_MANAGER,
        SPINEL_PROP_CHANNEL_MANAGER_SUPPORTED_CHANNELS, unpack_channel_mask);
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_ChannelManagerFavoredChannelMask,
                                                    SPINEL_CAP_CHANNEL_MANAGER,
                                                    SPINEL_PROP_CHANNEL_MANAGER_FAVORED_CHANNELS, unpack_channel_mask);
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_NCPCounterAllMac, SPINEL_CAP_COUNTERS,
                                                    SPINEL_PROP_CNTR_ALL_MAC_COUNTERS,
                                                    boost::bind(unpack_ncp_counters_all_mac, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_NCPCounterAllMacAsValMap, SPINEL_CAP_COUNTERS,
                                                    SPINEL_PROP_CNTR_ALL_MAC_COUNTERS,
                                                    boost::bind(unpack_ncp_counters_all_mac, _1, _2, _3, true));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_NCPCounterThreadMle, SPINEL_CAP_COUNTERS,
                                                    SPINEL_PROP_CNTR_MLE_COUNTERS,
                                                    boost::bind(unpack_ncp_counters_mle, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_NCPCounterThreadMleAsValMap, SPINEL_CAP_COUNTERS,
                                                    SPINEL_PROP_CNTR_MLE_COUNTERS,
                                                    boost::bind(unpack_ncp_counters_mle, _1, _2, _3, true));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_TimeSync_NetworkTime, SPINEL_CAP_TIME_SYNC,
                                                    SPINEL_PROP_THREAD_NETWORK_TIME,
                                                    boost::bind(unpack_thread_network_time_as_any, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(kWPANTUNDProperty_TimeSync_NetworkTimeAsValMap,
                                                    SPINEL_CAP_TIME_SYNC, SPINEL_PROP_THREAD_NETWORK_TIME,
                                                    boost::bind(unpack_thread_network_time_as_any, _1, _2, _3, true));
    register_get_handler_capability_spinel_unpacker(
        kWPANTUNDProperty_ThreadLeaderServices, SPINEL_CAP_THREAD_SERVICE, SPINEL_PROP_SERVER_LEADER_SERVICES,
        boost::bind(unpack_server_leader_services_as_any, _1, _2, _3, false));
    register_get_handler_capability_spinel_unpacker(
        kWPANTUNDProperty_ThreadLeaderServicesAsValMap, SPINEL_CAP_THREAD_SERVICE, SPINEL_PROP_SERVER_LEADER_SERVICES,
        boost::bind(unpack_server_leader_services_as_any, _1, _2, _3, true));

    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // Properties with a dedicated handler method

    register_get_handler(kWPANTUNDProperty_ConfigNCPDriverName,
                         boost::bind(&SpinelNCPInstance::get_prop_ConfigNCPDriverName, this, _1));
    register_get_handler(kWPANTUNDProperty_NCPCapabilities,
                         boost::bind(&SpinelNCPInstance::get_prop_NCPCapabilities, this, _1));
    register_get_handler(kWPANTUNDProperty_NetworkIsCommissioned,
                         boost::bind(&SpinelNCPInstance::get_prop_NetworkIsCommissioned, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadRouterID,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadRouterID, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadConfigFilterRLOCAddresses,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadConfigFilterRLOCAddresses, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadConfigFilterALOCAddresses,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadConfigFilterALOCAddresses, this, _1));
    register_get_handler(kWPANTUNDProperty_IPv6MeshLocalPrefix,
                         boost::bind(&SpinelNCPInstance::get_prop_IPv6MeshLocalPrefix, this, _1));
    register_get_handler(kWPANTUNDProperty_IPv6MeshLocalAddress,
                         boost::bind(&SpinelNCPInstance::get_prop_IPv6MeshLocalAddress, this, _1));
    register_get_handler(kWPANTUNDProperty_IPv6LinkLocalAddress,
                         boost::bind(&SpinelNCPInstance::get_prop_IPv6LinkLocalAddress, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadChildTable,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadChildTable, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadChildTableAsValMap,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadChildTableAsValMap, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadChildTableAddresses,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadChildTableAddresses, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadNeighborTable,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadNeighborTable, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadNeighborTableAsValMap,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadNeighborTableAsValMap, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadRouterTable,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadRouterTable, this, _1));
    register_get_handler(kWPANTUNDProperty_ThreadRouterTableAsValMap,
                         boost::bind(&SpinelNCPInstance::get_prop_ThreadRouterTableAsValMap, this, _1));
    register_get_handler(kWPANTUNDProperty_OpenThreadMsgBufferCounters,
                         boost::bind(&SpinelNCPInstance::get_prop_OpenThreadMsgBufferCounters, this, _1));
    register_get_handler(kWPANTUNDProperty_OpenThreadMsgBufferCountersAsString,
                         boost::bind(&SpinelNCPInstance::get_prop_OpenThreadMsgBufferCountersAsString, this, _1));
    register_get_handler(kWPANTUNDProperty_OpenThreadSteeringDataSetWhenJoinable,
                         boost::bind(&SpinelNCPInstance::get_prop_OpenThreadSteeringDataSetWhenJoinable, this, _1));
    register_get_handler(kWPANTUNDProperty_OpenThreadSteeringDataAddress,
                         boost::bind(&SpinelNCPInstance::get_prop_OpenThreadSteeringDataAddress, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetActiveTimestamp,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetActiveTimestamp, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetPendingTimestamp,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetPendingTimestamp, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetMasterKey,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetMasterKey, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetNetworkName,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetNetworkName, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetExtendedPanId,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetExtendedPanId, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetMeshLocalPrefix,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetMeshLocalPrefix, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetDelay,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetDelay, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetPanId,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetPanId, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetChannel,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetChannel, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetPSKc,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetPSKc, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetChannelMaskPage0,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetChannelMaskPage0, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetSecPolicyKeyRotation,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetSecPolicyKeyRotation, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetSecPolicyFlags,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetSecPolicyFlags, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetRawTlvs,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetRawTlvs, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetDestIpAddress,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetDestIpAddress, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetAllFileds,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetAllFileds, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetAllFileds_AltString,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetAllFileds, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetAllFiledsAsValMap,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetAllFiledsAsValMap, this, _1));
    register_get_handler(kWPANTUNDProperty_DatasetCommand,
                         boost::bind(&SpinelNCPInstance::get_prop_DatasetCommand, this, _1));
    register_get_handler(kWPANTUNDProperty_DaemonTickleOnHostDidWake,
    ...
    register_get_handler_capability(kWPANTUNDProperty_CommissionerEnergyScanResult, SPINEL_CAP_THREAD_COMMISSIONER,
                                    boost::bind(&SpinelNCPInstance::get_prop_CommissionerEnergyScanResult, this, _1));
    register_get_handler_capability(
        kWPANTUNDProperty_CommissionerPanIdConflictResult, SPINEL_CAP_THREAD_COMMISSIONER,
        boost::bind(&SpinelNCPInstance::get_prop_CommissionerPanIdConflictResult, this, _1));
    register_get_handler_capability(kWPANTUNDProperty_ThreadNeighborTableErrorRates, SPINEL_CAP_ERROR_RATE_TRACKING,
                                    boost::bind(&SpinelNCPInstance::get_prop_ThreadNeighborTableErrorRates, this, _1));
    register_get_handler_capability(
        kWPANTUNDProperty_ThreadNeighborTableErrorRatesAsValMap, SPINEL_CAP_ERROR_RATE_TRACKING,
        boost::bind(&SpinelNCPInstance::get_prop_ThreadNeighborTableErrorRatesAsValMap, this, _1));
    register_get_handler_capability(kWPANTUNDProperty_POSIXAppRCPVersionCached, SPINEL_CAP_POSIX_APP,
                                    boost::bind(&SpinelNCPInstance::get_prop_POSIXAppRCPVersionCached, this, _1));
    register_get_handler_capability(kWPANTUNDProperty_MACFilterFixedRssi, SPINEL_CAP_MAC_WHITELIST,
                                    boost::bind(&SpinelNCPInstance::get_prop_MACFilterFixedRssi, this, _1));
}

void SpinelNCPInstance::get_prop_ConfigNCPDriverName(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(std::string("spinel")));
}

void SpinelNCPInstance::get_prop_NCPCapabilities(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_NetworkIsCommissioned(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mIsCommissioned));
}

void SpinelNCPInstance::get_prop_ThreadRouterID(CallbackWithStatusArg1 cb)
{
    get_spinel_prop(boost::bind(convert_rloc16_to_router_id, cb, _1, _2), SPINEL_PROP_THREAD_RLOC16,
                    SPINEL_DATATYPE_UINT16_S);
}

void SpinelNCPInstance::get_prop_ThreadConfigFilterRLOCAddresses(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mFilterRLOCAddresses));
}

void SpinelNCPInstance::get_prop_ThreadConfigFilterALOCAddresses(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mFilterALOCAddresses));
}

void SpinelNCPInstance::get_prop_CommissionerEnergyScanResult(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mCommissionerEnergyScanResult));
}

void SpinelNCPInstance::get_prop_CommissionerPanIdConflictResult(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mCommissionerPanIdConflictResult));
}

void SpinelNCPInstance::get_prop_IPv6MeshLocalPrefix(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_IPv6MeshLocalAddress(CallbackWithStatusArg1 cb)
{
    if (!buffer_is_nonzero(mNCPV6Prefix, sizeof(mNCPV6Prefix)))
    {
...
void SpinelNCPInstance::get_prop_IPv6LinkLocalAddress(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_ThreadChildTable(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kChildTable,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_StringArray)));
}

void SpinelNCPInstance::get_prop_ThreadChildTableAsValMap(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kChildTable,
...
void SpinelNCPInstance::get_prop_ThreadChildTableAddresses(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kChildTableAddresses,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_StringArray)));
}

void SpinelNCPInstance::get_prop_ThreadNeighborTable(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kNeighborTable,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_StringArray)));
}

void SpinelNCPInstance::get_prop_ThreadNeighborTableAsValMap(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kNeighborTable,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_ValueMapArray)));
...
void SpinelNCPInstance::get_prop_ThreadNeighborTableErrorRates(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kNeighborTableErrorRates,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_StringArray)));
}

void SpinelNCPInstance::get_prop_ThreadNeighborTableErrorRatesAsValMap(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kNeighborTableErrorRates,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_ValueMapArray)));
...
void SpinelNCPInstance::get_prop_ThreadRouterTable(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kRouterTable,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_StringArray)));
}

void SpinelNCPInstance::get_prop_ThreadRouterTableAsValMap(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetNetworkTopology(this, cb, SpinelNCPTaskGetNetworkTopology::kRouterTable,
                                            SpinelNCPTaskGetNetworkTopology::kResultFormat_ValueMapArray)));
...
void SpinelNCPInstance::get_prop_OpenThreadMsgBufferCounters(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetMsgBufferCounters(this, cb, SpinelNCPTaskGetMsgBufferCounters::kResultFormat_StringArray)));
}

void SpinelNCPInstance::get_prop_OpenThreadMsgBufferCountersAsString(CallbackWithStatusArg1 cb)
{
    start_new_task(boost::shared_ptr<SpinelNCPTask>(
        new SpinelNCPTaskGetMsgBufferCounters(this, cb, SpinelNCPTaskGetMsgBufferCounters::kResultFormat_String)));
}

void SpinelNCPInstance::get_prop_OpenThreadSteeringDataSetWhenJoinable(CallbackWithStatusArg1 cb)
{
    cb(0, boost::any(mSetSteeringDataWhenJoinable));
}

void SpinelNCPInstance::get_prop_OpenThreadSteeringDataAddress(CallbackWithStatusArg1 cb)
{
    cb(0, boost::any(nl::Data(mSteeringDataAddress, sizeof(mSteeringDataAddress))));
}

void SpinelNCPInstance::get_prop_DatasetActiveTimestamp(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetPendingTimestamp(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetMasterKey(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetNetworkName(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetExtendedPanId(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetMeshLocalPrefix(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetDelay(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetPanId(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetChannel(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetPSKc(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetChannelMaskPage0(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetSecPolicyKeyRotation(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetSecPolicyFlags(CallbackWithStatusArg1 cb)
{
    if (mLocalDataset.mSecurityPolicy.has_value())
    {
...
void SpinelNCPInstance::get_prop_DatasetRawTlvs(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetDestIpAddress(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DatasetAllFileds(CallbackWithStatusArg1 cb)
{
    std::list<std::string> list;
...
void SpinelNCPInstance::get_prop_DatasetAllFiledsAsValMap(CallbackWithStatusArg1 cb)
{
    ValueMap map;
...
void SpinelNCPInstance::get_prop_DatasetCommand(CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::get_prop_DaemonTickleOnHostDidWake(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mTickleOnHostDidWake));
}

void SpinelNCPInstance::get_prop_POSIXAppRCPVersionCached(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mRcpVersion));
}

void SpinelNCPInstance::get_prop_MACFilterFixedRssi(CallbackWithStatusArg1 cb)
{
    cb(kWPANTUNDStatus_Ok, boost::any(mMacFilterFixedRssi));
}

void SpinelNCPInstance::property_get_value(const std::string &key, CallbackWithStatusArg1 cb)
{
...
void SpinelNCPInstance::set_spinel_prop(const boost::any & value,
                                        CallbackWithStatus cb,
                                        spinel_prop_key_t  prop_key,
                                        char               pack_type,
                                        unsigned int       capability,
                                        bool               save_in_settings,
                                        const std::string &prop_name)
{
    ...
    else
    {
        ...
        {
            start_new_task(SpinelNCPTaskSendCommand::Factory(this).set_callback(cb).add_command(command).finish());
        }
        else
        {
...
void SpinelNCPInstance::convert_value_prop_set(const boost::any & value,
                                               CallbackWithStatus cb,
                                               const std::string &prop_name,
...
void SpinelNCPInstance::register_set_handler(const char *prop_name, PropUpdateHandler handler, ValueConverter converter)
{
    if (converter.empty())
    {
    ...
    else
    {
        ...
        NCPInstanceBase::register_prop_set_handler(
            prop_name, boost::bind(&SpinelNCPInstance::convert_value_prop_set, _1, _2, _3, converter, handler));
    }
}

void SpinelNCPInstance::register_set_handler_spinel(const char *      prop_name,
                                                    spinel_prop_key_t prop_key,
                                                    char              pack_type,
                                                    ValueConverter    converter)
{
    register_set_handler(
        prop_name, boost::bind(&SpinelNCPInstance::set_spinel_prop, this, _1, _2, prop_key, pack_type, 0, false, _3),
        converter);
}

void SpinelNCPInstance::register_set_handler_spinel_persist(const char *      prop_name,
                                                            spinel_prop_key_t prop_key,
                                                            char              pack_type,
                                                            ValueConverter    converter)
{
    register_set_handler(
        prop_name, boost::bind(&SpinelNCPInstance::set_spinel_prop, this, _1, _2, prop_key, pack_type, 0, true, _3),
        converter);
}

void SpinelNCPInstance::register_set_handler_capability_spinel(const char *      prop_name,
                                                               unsigned int      capability,
                                                               spinel_prop_key_t prop_key,
                                                               char              pack_type,
                                                               ValueConverter    converter)
...
void SpinelNCPInstance::register_set_handler_capability_spinel_persist(const char *      prop_name,
                                                                       unsigned int      capability,
                                                                       spinel_prop_key_t prop_key,
                                                                       char              pack_type,
                                                                       ValueConverter    converter)
...
void SpinelNCPInstance::regsiter_all_set_handlers(void)
{
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    register_set_handler_spinel(kWPANTUNDProperty_NetworkPartitionId, SPINEL_PROP_NET_PARTITION_ID,
    ...
    register_set_handler_spinel(kWPANTUNDProperty_NCPExtendedAddress, SPINEL_PROP_MAC_EXTENDED_ADDR,
                                SPINEL_DATATYPE_EUI64_C);
    register_set_handler_spinel(kWPANTUNDProperty_NetworkKeyIndex, SPINEL_PROP_NET_KEY_SEQUENCE_COUNTER,
                                SPINEL_DATATYPE_UINT32_C);
    register_set_handler_spinel(kWPANTUNDProperty_NetworkKeySwitchGuardTime, SPINEL_PROP_NET_KEY_SWITCH_GUARDTIME,
                                SPINEL_DATATYPE_UINT32_C);
    ...
    register_set_handler_spinel(kWPANTUNDProperty_ThreadPreferredRouterID, SPINEL_PROP_THREAD_PREFERRED_ROUTER_ID,
                                SPINEL_DATATYPE_UINT8_C);
    register_set_handler_spinel(kWPANTUNDProperty_ThreadRouterRoleEnabled, SPINEL_PROP_THREAD_ROUTER_ROLE_ENABLED,
                                SPINEL_DATATYPE_BOOL_C);
    register_set_handler_spinel(kWPANTUNDProperty_ThreadRouterSelectionJitter,
                                SPINEL_PROP_THREAD_ROUTER_SELECTION_JITTER, SPINEL_DATATYPE_UINT8_C);
    register_set_handler_spinel(kWPANTUNDProperty_ThreadRouterUpgradeThreshold,
                                SPINEL_PROP_THREAD_ROUTER_UPGRADE_THRESHOLD, SPINEL_DATATYPE_UINT8_C);
    register_set_handler_spinel(kWPANTUNDProperty_ThreadRouterDowngradeThreshold,
                                SPINEL_PROP_THREAD_ROUTER_DOWNGRADE_THRESHOLD, SPINEL_DATATYPE_UINT8_C);
    register_set_handler_spinel(kWPANTUNDProperty_ThreadDomainName, SPINEL_PROP_THREAD_DOMAIN_NAME,
                                SPINEL_DATATYPE_UTF8_C);
    register_set_handler_spinel(kWPANTUNDProperty_ThreadBackboneInterface, SPINEL_PROP_THREAD_BACKBONE_INTERFACE,
                                SPINEL_DATATYPE_BOOL_C);
    register_set_handler_spinel(kWPANTUNDProperty_ThreadBackboneCoapPort, SPINEL_PROP_THREAD_BACKBONE_COAP_PORT,
    ...
    register_set_handler_spinel_persist(kWPANTUNDProperty_NCPCCAThreshold, SPINEL_PROP_PHY_CCA_THRESHOLD,
    ...
    register_set_handler_spinel_persist(kWPANTUNDProperty_ThreadChildTimeout, SPINEL_PROP_THREAD_CHILD_TIMEOUT,
                                        SPINEL_DATATYPE_UINT32_C);
    register_set_handler_spinel_persist(kWPANTUNDProperty_ThreadDeviceMode, SPINEL_PROP_THREAD_MODE,
                                        SPINEL_DATATYPE_UINT8_C);
    register_set_handler_spinel_persist(kWPANTUNDProperty_OpenThreadLogLevel, SPINEL_PROP_DEBUG_NCP_LOG_LEVEL,
                                        SPINEL_DATATYPE_UINT8_C);

    // Properties with a `ValueConverter`
    register_set_handler_spinel_persist(kWPANTUNDProperty_NCPChannelMask, SPINEL_PROP_PHY_CHAN_SUPPORTED,
                                        SPINEL_DATATYPE_DATA_C, &SpinelNCPInstance::convert_value_channel_mask);
    ...
    register_set_handler_capability_spinel(kWPANTUNDProperty_CommissionerProvisioningUrl,
    ...
    register_set_handler_capability_spinel_persist(kWPANTUNDProperty_NestLabs_LegacyMeshLocalPrefix,
                                                   SPINEL_CAP_NEST_LEGACY_INTERFACE, SPINEL_PROP_NEST_LEGACY_ULA_PREFIX,
                                                   SPINEL_DATATYPE_DATA_C);
    register_set_handler_capability_spinel_persist(kWPANTUNDProperty_ChildSupervisionInterval,
                                                   SPINEL_CAP_CHILD_SUPERVISION, SPINEL_PROP_CHILD_SUPERVISION_INTERVAL,
                                                   SPINEL_DATATYPE_UINT16_C);
    register_set_handler_capability_spinel_persist(
        kWPANTUNDProperty_ChildSupervisionCheckTimeout, SPINEL_CAP_CHILD_SUPERVISION,
        SPINEL_PROP_CHILD_SUPERVISION_CHECK_TIMEOUT, SPINEL_DATATYPE_UINT16_C);
    register_set_handler_capability_spinel_persist(kWPANTUNDProperty_ChannelManagerNewChannel,
                                                   SPINEL_CAP_CHANNEL_MANAGER, SPINEL_PROP_CHANNEL_MANAGER_NEW_CHANNEL,
                                                   SPINEL_DATATYPE_UINT8_C);
    ...
    register_set_handler_capability_spinel_persist(kWPANTUNDProperty_OpenThreadSLAACEnabled, SPINEL_CAP_SLAAC,
    ...
    register_set_handler_capability_spinel_persist(kWPANTUNDProperty_ChannelManagerFavoredChannelMask,
                                                   SPINEL_CAP_CHANNEL_MANAGER,
    ...
    register_set_handler(kWPANTUNDProperty_ThreadConfigFilterALOCAddresses,
    ...
    register_set_handler(kWPANTUNDProperty_UdpForwardStream,
    ...
    register_set_handler(kWPANTUNDProperty_ThreadDomainPrefix,
                         boost::bind(&SpinelNCPInstance::set_prop_ThreadDomainPrefix, this, _1, _2));
    register_set_handler(kWPANTUNDProperty_BbrSequenceNumber,
                         boost::bind(&SpinelNCPInstance::set_prop_BbrSequenceNumber, this, _1, _2));
    register_set_handler(kWPANTUNDProperty_BbrReregistrationDelay,
                         boost::bind(&SpinelNCPInstance::set_prop_BbrReregistrationDelay, this, _1, _2));
    register_set_handler(kWPANTUNDProperty_BbrMlrTimeout,
                         boost::bind(&SpinelNCPInstance::set_prop_BbrMlrTimeout, this, _1, _2));
    register_set_handler(kWPANTUNDProperty_ThreadLocalBbr,
...
int SpinelNCPInstance::convert_value_channel_mask(const boost::any &value, boost::any &value_out)
{
...
int SpinelNCPInstance::convert_value_CommissionerState(const boost::any &value, boost::any &value_out)
{
    int         ret       = kWPANTUNDStatus_Ok;
    std::string state_str = any_to_string(value);
    const char *str       = state_str.c_str();

    if (strcaseequal(str, kWPANTUNDCommissionerState_Disabled) || strcaseequal(str, "stop") ||
        strcaseequal(str, "off") || strcaseequal(str, "0") || strcaseequal(str, "false"))
    {
...
void SpinelNCPInstance::set_prop_NetworkKey(const boost::any &value, CallbackWithStatus cb)
{
    ...
    if (get_ncp_state() == CREDENTIALS_NEEDED)
    {
        ...
        start_new_task(boost::shared_ptr<SpinelNCPTask>(new SpinelNCPTaskJoin(this, boost::bind(cb, _1), options)));
    }
    else
    {
...
void SpinelNCPInstance::set_prop_InterfaceUp(const boost::any &value, CallbackWithStatus cb)
{
    ...
    if (isup)
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           ...
                           .add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_BOOL_S),
                                                       SPINEL_PROP_NET_IF_UP, true))
                           .add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_BOOL_S),
                                                       SPINEL_PROP_NET_STACK_UP, true))
                           .finish());
    }
    else
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           .set_callback(cb)
                           .add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_BOOL_S),
                                                       SPINEL_PROP_NET_STACK_UP, false))
                           .add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_BOOL_S),
                                                       SPINEL_PROP_NET_IF_UP, false))
                           .finish());
    }
}

void SpinelNCPInstance::set_prop_NetworkXPANID(const boost::any &value, CallbackWithStatus cb)
{
    mXPANIDWasExplicitlySet = true;
    set_spinel_prop(value, cb, SPINEL_PROP_NET_XPANID, SPINEL_DATATYPE_DATA_C);
}

void SpinelNCPInstance::set_prop_IPv6MeshLocalPrefix(const boost::any &value, CallbackWithStatus cb)
{
    ...
    start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                       ...
                       .add_command(SpinelPackData(
                           ...
                           SPINEL_PROP_IPV6_ML_PREFIX, &addr, 64))
                       .finish());
}

void SpinelNCPInstance::set_prop_ThreadConfigFilterRLOCAddresses(const boost::any &value, CallbackWithStatus cb)
{
    mFilterRLOCAddresses = any_to_bool(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_ThreadConfigFilterALOCAddresses(const boost::any &value, CallbackWithStatus cb)
{
    mFilterALOCAddresses = any_to_bool(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_OpenThreadSteeringDataSetWhenJoinable(const boost::any &value, CallbackWithStatus cb)
{
    mSetSteeringDataWhenJoinable = any_to_bool(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_OpenThreadSteeringDataAddress(const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::set_prop_TmfProxyStream(const boost::any &value, CallbackWithStatus cb)
{
    ...
    if (packet.size() > sizeof(uint16_t) * 2)
    {
        ...
        Data command =
            SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(
...
void SpinelNCPInstance::set_prop_UdpForwardStream(const boost::any &value, CallbackWithStatus cb)
{
    Data packet = any_to_data(value);
    ...
    if (packet.size() > sizeof(uint16_t) * 2 + sizeof(in6_addr))
    {
        ...
        Data command = SpinelPackData(
            SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_DATA_WLEN_S SPINEL_DATATYPE_UINT16_S   // Peer port
                                                                             SPINEL_DATATYPE_IPv6ADDR_S // Peer address
                                                                             SPINEL_DATATYPE_UINT16_S   // Sock port
                                                 ),
            SPINEL_PROP_THREAD_UDP_FORWARD_STREAM, packet.data(), payload_len, peer_port, &peer_addr, sock_port);
...
void SpinelNCPInstance::set_prop_DatasetActiveTimestamp(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mActiveTimestamp = any_to_uint64(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetPendingTimestamp(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mPendingTimestamp = any_to_uint64(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetMasterKey(const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::set_prop_DatasetNetworkName(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mNetworkName = any_to_string(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetExtendedPanId(const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::set_prop_DatasetMeshLocalPrefix(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mMeshLocalPrefix = any_to_ipv6(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetDelay(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mDelay = static_cast<uint32_t>(any_to_int(value));
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetPanId(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mPanId = static_cast<uint16_t>(any_to_int(value));
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetChannel(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mChannel = static_cast<uint8_t>(any_to_int(value));
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetPSKc(const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::set_prop_DatasetChannelMaskPage0(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mChannelMaskPage0 = static_cast<uint32_t>(any_to_int(value));
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetSecPolicyKeyRotation(const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::set_prop_DatasetSecPolicyFlags(const boost::any &value, CallbackWithStatus cb)
{
    ThreadDataset::SecurityPolicy policy = mLocalDataset.mSecurityPolicy.get();
...
void SpinelNCPInstance::set_prop_DatasetRawTlvs(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mRawTlvs = any_to_data(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetDestIpAddress(const boost::any &value, CallbackWithStatus cb)
{
    mLocalDataset.mDestIpAddress = any_to_ipv6(value);
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_DatasetCommand(const boost::any &value, CallbackWithStatus cb)
{
    perform_dataset_command(any_to_string(value), cb);
}

void SpinelNCPInstance::set_prop_ThreadDomainPrefix(const boost::any &value, CallbackWithStatus cb)
{
    Data packet = any_to_data(value);
    ...
    if (packet.size() >= sizeof(uint8_t) * 2 + sizeof(in6_addr))
    {
        ...
        factory.add_command(
            SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_IPv6ADDR_S      // Prefix
                                                                        SPINEL_DATATYPE_UINT8_S // Prefix length
                                                                        SPINEL_DATATYPE_UINT8_S // Prefix flags
                                                                ),
                           SPINEL_PROP_THREAD_DOMAIN_PREFIX, &prefix, prefixLen, flags));
...
void SpinelNCPInstance::set_prop_BbrSequenceNumber(const boost::any &value, CallbackWithStatus cb)
{
    mBbrDataset.mSequenceNumber.set(static_cast<uint8_t>(any_to_int(value)));
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_BbrReregistrationDelay(const boost::any &value, CallbackWithStatus cb)
{
    mBbrDataset.mReregistrationDelay.set(static_cast<uint16_t>(any_to_int(value)));
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_BbrMlrTimeout(const boost::any &value, CallbackWithStatus cb)
{
    mBbrDataset.mMlrTimeout.set(static_cast<uint32_t>(any_to_int(value)));
    cb(kWPANTUNDStatus_Ok);
}

void SpinelNCPInstance::set_prop_ThreadLocalBbr(const boost::any &value, CallbackWithStatus cb)
{
    ...
    if (onoff)
    {
        ...
        factory.add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_DATA_S),
                                           SPINEL_PROP_THREAD_BACKBONE_ROUTER_LOCAL, frame.data(), frame.size()));
...
void SpinelNCPInstance::set_prop_DaemonTickleOnHostDidWake(const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::set_prop_MACFilterFixedRssi(const boost::any &value, CallbackWithStatus cb)
{
    if (mCapabilities.count(SPINEL_CAP_MAC_WHITELIST))
    {
...
void SpinelNCPInstance::check_capability_prop_update(const boost::any & value,
                                                     CallbackWithStatus cb,
                                                     const std::string &prop_name,
                                                     unsigned int       capability,
                                                     PropUpdateHandler  handler)
...
void SpinelNCPInstance::register_insert_handler(const char *prop_name, PropUpdateHandler handler)
{
    NCPInstanceBase::register_prop_insert_handler(prop_name, handler);
}

void SpinelNCPInstance::register_insert_handler_capability(const char *      prop_name,
                                                           unsigned int      capability,
                                                           PropUpdateHandler handler)
{
    register_insert_handler(prop_name, boost::bind(&SpinelNCPInstance::check_capability_prop_update, this, _1, _2, _3,
                                                   capability, handler));
}

void SpinelNCPInstance::regsiter_all_insert_handlers(void)
{
    register_insert_handler_capability(kWPANTUNDProperty_MACWhitelistEntries, SPINEL_CAP_MAC_WHITELIST,
                                       boost::bind(&SpinelNCPInstance::insert_prop_MACWhitelistEntries, this, _1, _2));
    register_insert_handler_capability(kWPANTUNDProperty_MACBlacklistEntries, SPINEL_CAP_MAC_WHITELIST,
                                       boost::bind(&SpinelNCPInstance::insert_prop_MACBlacklistEntries, this, _1, _2));
    register_insert_handler_capability(kWPANTUNDProperty_MACFilterEntries, SPINEL_CAP_MAC_WHITELIST,
                                       boost::bind(&SpinelNCPInstance::insert_prop_MACFilterEntries, this, _1, _2));
}

void SpinelNCPInstance::insert_prop_MACWhitelistEntries(const boost::any &value, CallbackWithStatus cb)
{
    ...
    if (ext_address.size() == sizeof(spinel_eui64_t))
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           ...
                           .add_command(SpinelPackData(
                               ...
                               SPINEL_PROP_MAC_WHITELIST, ext_address.data(), rssi))
                           .finish());
    }
    else
    {
...
void SpinelNCPInstance::insert_prop_MACBlacklistEntries(const boost::any &value, CallbackWithStatus cb)
{
    Data   ext_address = any_to_data(value);
    int8_t rssi        = kWPANTUND_Whitelist_RssiOverrideDisabled;
    ...
    if (ext_address.size() == sizeof(spinel_eui64_t))
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           ...
                           .add_command(SpinelPackData(
                               SPINEL_FRAME_PACK_CMD_PROP_VALUE_INSERT(SPINEL_DATATYPE_EUI64_S SPINEL_DATATYPE_INT8_S),
                               SPINEL_PROP_MAC_BLACKLIST, ext_address.data(), rssi))
                           .finish());
    }
    else
    {
...
void SpinelNCPInstance::insert_prop_MACFilterEntries(const boost::any &value, CallbackWithStatus cb)
{
    Data ext_address = any_to_data(value);
    ...
    if (ext_address.size() == sizeof(spinel_eui64_t))
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           ...
                           .add_command(SpinelPackData(
                               SPINEL_FRAME_PACK_CMD_PROP_VALUE_INSERT(SPINEL_DATATYPE_EUI64_S SPINEL_DATATYPE_INT8_S),
                               SPINEL_PROP_MAC_FIXED_RSS, ext_address.data(), mMacFilterFixedRssi))
                           .finish());
    }
    else
    {
...
void SpinelNCPInstance::property_insert_value(const std::string &key, const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::register_remove_handler(const char *prop_name, PropUpdateHandler handler)
{
    NCPInstanceBase::register_prop_remove_handler(prop_name, handler);
}

void SpinelNCPInstance::register_remove_handler_capability(const char *      prop_name,
                                                           unsigned int      capability,
                                                           PropUpdateHandler handler)
{
    register_remove_handler(prop_name, boost::bind(&SpinelNCPInstance::check_capability_prop_update, this, _1, _2, _3,
                                                   capability, handler));
}

void SpinelNCPInstance::regsiter_all_remove_handlers(void)
{
    register_remove_handler_capability(kWPANTUNDProperty_MACWhitelistEntries, SPINEL_CAP_MAC_WHITELIST,
                                       boost::bind(&SpinelNCPInstance::remove_prop_MACWhitelistEntries, this, _1, _2));
    register_remove_handler_capability(kWPANTUNDProperty_MACBlacklistEntries, SPINEL_CAP_MAC_WHITELIST,
                                       boost::bind(&SpinelNCPInstance::remove_prop_MACBlacklistEntries, this, _1, _2));
    register_remove_handler_capability(kWPANTUNDProperty_MACFilterEntries, SPINEL_CAP_MAC_WHITELIST,
                                       boost::bind(&SpinelNCPInstance::remove_prop_MACFilterEntries, this, _1, _2));
}

void SpinelNCPInstance::remove_prop_MACWhitelistEntries(const boost::any &value, CallbackWithStatus cb)
{
    Data ext_address = any_to_data(value);
    ...
    if (ext_address.size() == sizeof(spinel_eui64_t))
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           .set_callback(cb)
                           .add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_REMOVE(SPINEL_DATATYPE_EUI64_S),
                                                       SPINEL_PROP_MAC_WHITELIST, ext_address.data()))
                           .finish());
    }
    else
    {
...
void SpinelNCPInstance::remove_prop_MACBlacklistEntries(const boost::any &value, CallbackWithStatus cb)
{
    Data ext_address = any_to_data(value);
    ...
    if (ext_address.size() == sizeof(spinel_eui64_t))
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           .set_callback(cb)
                           .add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_REMOVE(SPINEL_DATATYPE_EUI64_S),
                                                       SPINEL_PROP_MAC_BLACKLIST, ext_address.data()))
                           .finish());
    }
    else
    {
...
void SpinelNCPInstance::remove_prop_MACFilterEntries(const boost::any &value, CallbackWithStatus cb)
{
    Data ext_address = any_to_data(value);
    ...
    if (ext_address.size() == sizeof(spinel_eui64_t))
    {
        start_new_task(SpinelNCPTaskSendCommand::Factory(this)
                           .set_callback(cb)
                           .add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_REMOVE(SPINEL_DATATYPE_EUI64_S),
                                                       SPINEL_PROP_MAC_FIXED_RSS, ext_address.data()))
                           .finish());
    }
    else
    {
...
void SpinelNCPInstance::property_remove_value(const std::string &key, const boost::any &value, CallbackWithStatus cb)
{
...
void SpinelNCPInstance::reset_tasks(wpantund_status_t status)
{
...
void SpinelNCPInstance::handle_ncp_spinel_value_is_ON_MESH_NETS(const uint8_t *value_data_ptr,
                                                                spinel_size_t  value_data_len)
{
    ...
    while (value_data_len > 0)
    {
        ...
        len = spinel_datatype_unpack(value_data_ptr, value_data_len,
                                     SPINEL_DATATYPE_STRUCT_S(SPINEL_DATATYPE_IPv6ADDR_S                  // Prefix
                                     ...
                                     &prefix_addr, &prefix_len, &stable, &flags, &is_local, &rloc16);
        ...
        syslog(LOG_INFO, "[-NCP-]: On-mesh net [%d] \"%s/%d\" stable:%s local:%s flags:%s, rloc16:0x%04x", num_prefix,
               in6_addr_to_string(*prefix_addr).c_str(), prefix_len, stable ? "yes" : "no", is_local ? "yes" : "no",
               on_mesh_prefix_flags_to_string(flags).c_str(), rloc16);
        ...
        if (!is_local)
        {
            ...
            {
                std::multimap<IPv6Prefix, OnMeshPrefixEntry>::iterator upper_iter =
                    on_mesh_prefixes.upper_bound(prefix);
    ...
    for (iter = on_mesh_prefixes.begin(); iter != on_mesh_prefixes.end(); ++iter)
    {
        ...
        {
            on_mesh_prefix_was_removed(kOriginThreadNCP, iter->first.get_prefix(), iter->first.get_length(),
                                       iter->second.get_flags(), iter->second.is_stable(), iter->second.get_rloc());
        }
    }
}

void SpinelNCPInstance::handle_ncp_spinel_value_is_OFF_MESH_ROUTES(const uint8_t *value_data_ptr,
                                                                   spinel_size_t  value_data_len)
{
    ...
    while (value_data_len > 0)
    {
        ...
        len = spinel_datatype_unpack(
            value_data_ptr, value_data_len,
            SPINEL_DATATYPE_STRUCT_S(SPINEL_DATATYPE_IPv6ADDR_S                       // Route Prefix
                                                             SPINEL_DATATYPE_UINT8_S  // Prefix Length (in bits)
            ...
            &route_prefix, &prefix_len, &is_stable, &flags, &is_local, &next_hop_is_this_device, &rloc16);
        ...
        syslog(LOG_INFO,
               "[-NCP-]: Off-mesh route [%d] \"%s/%d\" stable:%s local:%s preference:%s, next_hop_is_this_device:%s, "
               "rloc16:0x%0x",
...
void SpinelNCPInstance::handle_ncp_spinel_value_is_SERVICES(const uint8_t *value_data_ptr, spinel_size_t value_data_len)
{
    uint32_t       enterprise_number;
    ...
    {
        len = spinel_datatype_unpack(value_data_ptr, value_data_len,
                                     SPINEL_DATATYPE_STRUCT_S(SPINEL_DATATYPE_UINT32_S // Enterprise Number
                                                                              SPINEL_DATATYPE_DATA_WLEN_S // Service Data
                                                                              SPINEL_DATATYPE_BOOL_S      // stable
                                                                              SPINEL_DATATYPE_DATA_WLEN_S // Server Data
                                                                              SPINEL_DATATYPE_UINT16_S    // RLOC
                                                              ),
                                     &enterprise_number, &service_data_ptr, &service_data_len, &stable,
                                     &server_data_ptr, &server_data_len, &rloc16);
        ...
        syslog(LOG_INFO, "[-NCP-]: Service [%d] enterprise_number:%u stable:%s RLOC16:%04x", num_services,
               enterprise_number, stable ? "yes" : "no", rloc16);
...
void SpinelNCPInstance::handle_ncp_spinel_value_is(spinel_prop_key_t key,
                                                   ...
                                                   spinel_size_t     value_data_len)
{
    ...
    else if (key == SPINEL_PROP_PROTOCOL_VERSION)
    {
        ...
        {
            syslog(LOG_CRIT, "[-NCP-]: NCP is using unsupported protocol version (NCP:%d, wpantund:%d)",
                   protocol_version_major, SPINEL_PROTOCOL_VERSION_THREAD_MAJOR);
            change_ncp_state(FAULT);
        }

        if (protocol_version_minor != SPINEL_PROTOCOL_VERSION_THREAD_MINOR)
        {
            syslog(LOG_WARNING, "[-NCP-]: NCP is using different protocol minor version (NCP:%d, wpantund:%d)",
                   protocol_version_minor, SPINEL_PROTOCOL_VERSION_THREAD_MINOR);
        }
    }
    else if (key == SPINEL_PROP_CAPS)
    {
        ...
        while (data_len != 0)
        {
            ...
            spinel_ssize_t parse_len =
                spinel_datatype_unpack(data_ptr, data_len, SPINEL_DATATYPE_UINT_PACKED_S, &value);
            if (parse_len <= 0)
            {
    ...
    else if (key == SPINEL_PROP_IPV6_MULTICAST_ADDRESS_TABLE)
    {
        ...
        while (value_data_len > 0)
        {
            ...
            addr = reinterpret_cast<const struct in6_addr *>(entry_ptr);
            syslog(LOG_INFO, "[-NCP-]: Multicast IPv6 address [%d] \"%s\"", num_address,
    ...
    else if (key == SPINEL_PROP_NET_ROLE)
    {
        ...
        if (ret > 0)
        {
            ...
            if (ncp_state_is_joining_or_joined(get_ncp_state()) && (value != SPINEL_NET_ROLE_DETACHED))
            {
    ...
    else if (key == SPINEL_PROP_STREAM_RAW)
    {
        if (mPcapManager.is_enabled())
        {
            ...
            // Unpack the packet.
            ret = spinel_datatype_unpack(value_data_ptr, value_data_len,
                                         SPINEL_DATATYPE_DATA_WLEN_S SPINEL_DATATYPE_DATA_S, &frame_ptr, &frame_len,
                                         &meta_ptr, &meta_len);

            require(ret > 0, bail);

            // Unpack the metadata.
            ret = spinel_datatype_unpack(meta_ptr, meta_len,
            ...
            mPcapManager.push_packet(
                packet.append_ppi_field(PCAP_PPI_TYPE_SPINEL, meta_ptr, meta_len).append_payload(frame_ptr, frame_len));
        }
    }
    else if (key == SPINEL_PROP_THREAD_TMF_PROXY_STREAM)
    {
        ...
        ret = spinel_datatype_unpack(value_data_ptr, value_data_len,
                                     ...
                                     &frame_ptr, &frame_len, &locator, &port);
    ...
    else if (key == SPINEL_PROP_THREAD_UDP_FORWARD_STREAM)
    {
        ...
        ret = spinel_datatype_unpack(value_data_ptr, value_data_len,
                                     SPINEL_DATATYPE_DATA_S SPINEL_DATATYPE_UINT16_S   // Peer port
                                                            SPINEL_DATATYPE_IPv6ADDR_S // Peer address
                                                            SPINEL_DATATYPE_UINT16_S,  // Sock port
                                     &frame_ptr, &frame_len, &peer_port, &peer_addr, &sock_port);
    ...
    else if ((key == SPINEL_PROP_STREAM_NET) || (key == SPINEL_PROP_STREAM_NET_INSECURE))
    {
        ...
        ret = spinel_datatype_unpack(value_data_ptr, value_data_len, SPINEL_DATATYPE_DATA_S SPINEL_DATATYPE_DATA_S,
                                     &frame_ptr, &frame_len, NULL, NULL);
    ...
    else if (key == SPINEL_PROP_THREAD_GROUP_TABLE)
    {
        ...
        while (value_data_len > 0)
        {
            ...
            len = spinel_datatype_unpack(value_data_ptr, value_data_len, SPINEL_DATATYPE_DATA_WLEN_S, &struct_data,
                                         &struct_len);
            ...
            len = spinel_datatype_unpack(struct_data, struct_len,
                                         (SPINEL_DATATYPE_IPv6ADDR_S   // Multicast IP address
                                              SPINEL_DATATYPE_UINT32_S // Expiration time
                                          ),
                                         &ip6_addr, &expirationtime);

            __ASSERT_MACROS_check(len > 0);

            syslog(LOG_INFO,
                   "[-NCP-]: Group %02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X"
                   "(Expiration Time %u)",
    ...
    else if (key == SPINEL_PROP_THREAD_NDPROXY_TABLE)
    {
...

        {
            ...
            while (value_data_len > 0)
            {
                ...
                len =
                    spinel_datatype_unpack(struct_data, struct_len,
                                           (SPINEL_DATATYPE_IPv6ADDR_S SPINEL_DATATYPE_EUI64_S SPINEL_DATATYPE_UINT32_S
                                                SPINEL_DATATYPE_BOOL_S SPINEL_DATATYPE_BOOL_S),
                                           &ip6_addr, &eui64, &timeSinceLastRegistration, &dadFlag, &arpCacheFlag);
                ...
                syslog(LOG_INFO, "[-NCP-]: EUI64 %02X%02X%02X%02X%02X%02X%02X%02X", eui64->bytes[0], eui64->bytes[1],
                       eui64->bytes[2], eui64->bytes[3], eui64->bytes[4], eui64->bytes[5], eui64->bytes[6],
                       eui64->bytes[7]);
            ...
            syslog(LOG_INFO, "[-NCP-] NdProxyTable : Total %d NdProxy Entr%s", num_entries,
                   (num_entries > 1) ? "ies" : "y");
    ...
    else if (key == SPINEL_PROP_THREAD_NETWORK_TIME)
    {
        ...
        {
            if (unpack_thread_network_time_as_string(value_data_ptr, value_data_len, result_as_string) ==
                kWPANTUNDStatus_Ok)
            {
...
void SpinelNCPInstance::handle_ncp_spinel_value_inserted(spinel_prop_key_t key,
                                                         const uint8_t *   value_data_ptr,
                                                         spinel_size_t     value_data_len)
{
    if (key == SPINEL_PROP_IPV6_ADDRESS_TABLE)
    {
        ...
        spinel_datatype_unpack(value_data_ptr, value_data_len, "6CLL", &addr, &prefix_len, &valid_lifetime,
                               &preferred_lifetime);
    ...
    else if (key == SPINEL_PROP_THREAD_GROUP_TABLE)
    {
...

        {
            ...
            while (value_data_len > 0)
            {
                len = spinel_datatype_unpack(value_data_ptr, value_data_len, SPINEL_DATATYPE_IPv6ADDR_S, &ip6_addr);
    ...
    else if (key == SPINEL_PROP_MESHCOP_COMMISSIONER_ENERGY_SCAN_RESULT)
    {
        ...
        len = spinel_datatype_unpack(value_data_ptr, value_data_len,
                                     (SPINEL_DATATYPE_UINT32_S SPINEL_DATATYPE_DATA_WLEN_S), &channel_mask,
                                     &energy_data, &energy_data_len);
    ...
    else if (key == SPINEL_PROP_MESHCOP_COMMISSIONER_PAN_ID_CONFLICT_RESULT)
    {
        ...
        len = spinel_datatype_unpack(value_data_ptr, value_data_len,
                                     (SPINEL_DATATYPE_UINT16_S SPINEL_DATATYPE_UINT32_S), &panid, &channel_mask);
...
void SpinelNCPInstance::handle_ncp_spinel_value_removed(spinel_prop_key_t key,
                                                        const uint8_t *   value_data_ptr,
                                                        spinel_size_t     value_data_len)
{
...
void SpinelNCPInstance::handle_ncp_state_change(NCPState new_ncp_state, NCPState old_ncp_state)
{
    NCPInstanceBase::handle_ncp_state_change(new_ncp_state, old_ncp_state);

    if (ncp_state_is_joining_or_joined(old_ncp_state) && (new_ncp_state == OFFLINE))
    {
    ...
    if (ncp_state_is_associated(new_ncp_state) && !ncp_state_is_associated(old_ncp_state))
    {
        mIsCommissioned = true;
        start_new_task(
            SpinelNCPTaskSendCommand::Factory(this)
    ...
    else if (ncp_state_is_joining(new_ncp_state) && !ncp_state_is_joining(old_ncp_state))
    {
...
void SpinelNCPInstance::handle_ncp_spinel_callback(unsigned int   command,
                                                   ...
                                                   spinel_size_t  cmd_data_len)
{
    ...
    case SPINEL_CMD_PROP_VALUE_REMOVED:
    {
        ...
        ret = spinel_datatype_unpack(cmd_data_ptr, cmd_data_len, "CiiD", NULL, NULL, &key, &value_data_ptr,
                                     &value_data_len);
...
bool SpinelNCPInstance::should_filter_address(const struct in6_addr &addr, uint8_t prefix_len)
{
...
void SpinelNCPInstance::filter_addresses(void)
{
    std::map<struct in6_addr, UnicastAddressEntry>           unicast_addresses(mUnicastAddresses);
...
void SpinelNCPInstance::add_unicast_address_on_ncp(const struct in6_addr &addr,
                                                   uint8_t                prefix_len,
                                                   CallbackWithStatus     cb)
{
    SpinelNCPTaskSendCommand::Factory factory(this);
    ...
    factory.add_command(SpinelPackData(
        SPINEL_FRAME_PACK_CMD_PROP_VALUE_INSERT(SPINEL_DATATYPE_IPv6ADDR_S           // Address
                                                            SPINEL_DATATYPE_UINT8_S  // Prefix Length
                                                            SPINEL_DATATYPE_UINT32_S // Valid Lifetime
                                                            SPINEL_DATATYPE_UINT32_S // Preferred Lifetime
                                                ),
        SPINEL_PROP_IPV6_ADDRESS_TABLE, &addr, prefix_len, UINT32_MAX, UINT32_MAX));

    start_new_task(factory.finish());
}

void SpinelNCPInstance::remove_unicast_address_on_ncp(const struct in6_addr &addr,
                                                      uint8_t                prefix_len,
                                                      CallbackWithStatus     cb)
{
    SpinelNCPTaskSendCommand::Factory factory(this);
    ...
    factory.add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_REMOVE(SPINEL_DATATYPE_IPv6ADDR_S  // Address
                                                                                   SPINEL_DATATYPE_UINT8_S // Prefix
                                                                               ),
                                       SPINEL_PROP_IPV6_ADDRESS_TABLE, &addr, prefix_len));

    start_new_task(factory.finish());
}

void SpinelNCPInstance::add_multicast_address_on_ncp(const struct in6_addr &addr, CallbackWithStatus cb)
{
    SpinelNCPTaskSendCommand::Factory factory(this);
    ...
    factory.add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_INSERT(SPINEL_DATATYPE_IPv6ADDR_S // Address
                                                                               ),
                                       SPINEL_PROP_IPV6_MULTICAST_ADDRESS_TABLE, &addr));

    start_new_task(factory.finish());
}

void SpinelNCPInstance::remove_multicast_address_on_ncp(const struct in6_addr &addr, CallbackWithStatus cb)
{
    SpinelNCPTaskSendCommand::Factory factory(this);
...
void SpinelNCPInstance::add_service_on_ncp(uint32_t           enterprise_number,
                                           ...
                                           CallbackWithStatus cb)
{
    ...
    if (mCapabilities.count(SPINEL_CAP_THREAD_SERVICE) > 0)
    {
        ...
        factory.add_command(SpinelPackData(
            SPINEL_FRAME_PACK_CMD_PROP_VALUE_INSERT(SPINEL_DATATYPE_UINT32_S                // Enterprise Number
                                                                SPINEL_DATATYPE_DATA_WLEN_S // Service Data
                                                                SPINEL_DATATYPE_BOOL_S      // stable
                                                                SPINEL_DATATYPE_DATA_WLEN_S // Server Data
                                                    ),
            SPINEL_PROP_SERVER_SERVICES, enterprise_number, service_data.data(), service_data.size(), stable,
            server_data.data(), server_data.size()));
...
void SpinelNCPInstance::remove_service_on_ncp(uint32_t           enterprise_number,
                                              const Data &       service_data,
                                              CallbackWithStatus cb)
{
    SpinelNCPTaskSendCommand::Factory factory(this);
    ...
    if (mCapabilities.count(SPINEL_CAP_THREAD_SERVICE) > 0)
    {
        ...
        factory.add_command(
            SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_REMOVE(SPINEL_DATATYPE_UINT32_S        // Enterprise Number
                                                                       SPINEL_DATATYPE_DATA_WLEN_S // Service Data
                                                                   ),
                           SPINEL_PROP_SERVER_SERVICES, enterprise_number, service_data.data(), service_data.size()));
...
void SpinelNCPInstance::add_on_mesh_prefix_on_ncp(const struct in6_addr &prefix,
                                                  uint8_t                prefix_len,
                                                  uint8_t                flags,
                                                  bool                   stable,
                                                  CallbackWithStatus     cb)
{
    ...
    factory.add_command(SpinelPackData(
        SPINEL_FRAME_PACK_CMD_PROP_VALUE_INSERT(
            SPINEL_DATATYPE_IPv6ADDR_S SPINEL_DATATYPE_UINT8_S SPINEL_DATATYPE_BOOL_S SPINEL_DATATYPE_UINT8_S),
        SPINEL_PROP_THREAD_ON_MESH_NETS, &prefix, prefix_len, stable, flags));

    start_new_task(factory.finish());
}

void SpinelNCPInstance::remove_on_mesh_prefix_on_ncp(const struct in6_addr &prefix,
                                                     uint8_t                prefix_len,
                                                     uint8_t                flags,
                                                     bool                   stable,
                                                     CallbackWithStatus     cb)
...
void SpinelNCPInstance::add_route_on_ncp(const struct in6_addr &route,
                                         uint8_t                prefix_len,
                                         RoutePreference        preference,
                                         bool                   stable,
                                         CallbackWithStatus     cb)
{
    ...
    factory.add_command(SpinelPackData(
        SPINEL_FRAME_PACK_CMD_PROP_VALUE_INSERT(
            SPINEL_DATATYPE_IPv6ADDR_S SPINEL_DATATYPE_UINT8_S SPINEL_DATATYPE_BOOL_S SPINEL_DATATYPE_UINT8_S),
        SPINEL_PROP_THREAD_OFF_MESH_ROUTES, &route, prefix_len, stable, convert_route_preference_to_flags(preference)));

    start_new_task(factory.finish());
}

void SpinelNCPInstance::remove_route_on_ncp(const struct in6_addr &route,
                                            uint8_t                prefix_len,
                                            RoutePreference        preference,
                                            bool                   stable,
                                            CallbackWithStatus     cb)
{
    ...
    syslog(LOG_NOTICE, "Removing off-mesh route \"%s/%d\" with preference %s from NCP",
           in6_addr_to_string(route).c_str(), prefix_len,
           NCPControlInterface::external_route_priority_to_string(preference).c_str());
...
SpinelNCPInstance::RoutePreference SpinelNCPInstance::convert_flags_to_route_preference(uint8_t flags)
{
...
uint8_t SpinelNCPInstance::convert_route_preference_to_flags(RoutePreference preference)
{
...
void SpinelNCPInstance::log_spinel_frame(SpinelFrameOrigin origin, const uint8_t *frame_ptr, spinel_size_t frame_len)
{
    ...
    if (logmask & LOG_MASK(LOG_INFO))
    {
        ...
        read_len =
            spinel_datatype_unpack(frame_ptr, frame_len, "CiD", &header, &command, &cmd_payload_ptr, &cmd_payload_len);
        ...
        case SPINEL_CMD_PROP_VALUE_REMOVED:
        {
            ...
            read_len =
                spinel_datatype_unpack(cmd_payload_ptr, cmd_payload_len, "iD", &prop_key, &value_ptr, &value_len);
            require_quiet(read_len > 0, bail);
...
bool SpinelNCPInstance::is_busy(void)
{
    return NCPInstanceBase::is_busy() || !mTaskQueue.empty();
}

void SpinelNCPInstance::process(void)
{
    ...
    if (!is_initializing_ncp() && mTaskQueue.empty())
    {
        ...
        if (mIsPcapInProgress != x)
        {
            ...
            factory.add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_BOOL_S),
                                               SPINEL_PROP_MAC_RAW_STREAM_ENABLED, mIsPcapInProgress));
            ...
            if (mIsPcapInProgress)
            {
                ...
                if (!ncp_state_is_joining_or_joined(get_ncp_state()))
                {
                    factory.add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_UINT8_S),
            ...
            {
                factory.add_command(SpinelPackData(SPINEL_FRAME_PACK_CMD_PROP_VALUE_SET(SPINEL_DATATYPE_UINT8_S),
                                                   SPINEL_PROP_MAC_PROMISCUOUS_MODE, SPINEL_MAC_PROMISCUOUS_MODE_OFF));
            }
