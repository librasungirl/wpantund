...
SpinelNCPInstance::regsiter_all_get_handlers(void)
{
	...
	register_get_handler_spinel_simple(
			...
			SPINEL_PROP_THREAD_DOMAIN_PREFIX, SPINEL_DATATYPE_DATA_S);
	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
SpinelNCPInstance::handle_ncp_spinel_value_is(spinel_prop_key_t key, const uint8_t* value_data_ptr, spinel_size_t value_data_len)
{
	...
	if (key == SPINEL_PROP_LAST_STATUS) {
		...
		uint8_t prefixLen = *(value_data_ptr + sizeof(struct in6_addr));
		memset(&prefix_addr, 0, sizeof(struct in6_addr));
		memcpy(&prefix_addr, value_data_ptr, sizeof(struct in6_addr));

		syslog(LOG_INFO, "[-NCP-] Domain Prefix %s/%d", in6_addr_to_string(prefix_addr).c_str(), prefixLen);

		if ((prefixLen == 64) && 0 != memcmp(value_data_ptr, mDomainPrefix, 8)) {
			memcpy(mDomainPrefix, value_data_ptr, 8);
